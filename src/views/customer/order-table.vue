    ...
    >
      ...
      <el-table-column label="会员" min-width="190px">
        <template slot-scope="{row}">
          <span>会员ID：{{ row.vipId }}<br></span>
          <span>会员电话：{{ row.vipNumber }}    </span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="退款" min-width="180px">
        <template slot-scope="{row}">
          <!--  <span  >退款状态：{{ row.orderBackType }}<br></span>-->
          <!-- <el-tag>{{ row.backMoneyStatus | backMoneyStatusFilter }}</el-tag>-->
          <span>退款状态：{{ row.backMoneyStatus | backMoneyStatusFilter }}<br></span>
          <span>退款金额：{{ row.backMoney }}    </span>
        </template>
      </el-table-column>
...
<script>
import { fetchList, createBike, refundMoney, backEbikeByManual } from '@/api/order'
